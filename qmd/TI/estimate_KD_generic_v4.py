...
if SCRIPT_MODE <= 0:
    ...
    # similarly define array_Xw_{secondary_species} -- initialize with NaN, will fill later
    ...
    def compute_KD__exact(row,df_evaluate=None):
        ...
        # fn_estimate__X_in_MgSiO3 # i.e. estimating array_X_{secondary_species} (MgSiO3)
        if phase == f"Fe_{secondary_species}":
            array_X_in_Fe = row[f"array_X_{secondary_species}"]
        else:
            array_X_in_Fe = df_evaluate.loc[mask, f"array_X_{secondary_species}"].iloc[0]

        fn_estimate__X_in_MgSiO3 = lambda array_X_in_Fe, KD__part: 5 / ( (KD__part/array_X_in_Fe) + 4)
        array_X_in_MgSiO3 = fn_estimate__X_in_MgSiO3(
            array_X_in_Fe, KD__part
        )
        if phase == f"Fe_{secondary_species}":
    ...
    for i, row in df.iterrows():
        ...
        if phase == f"Fe_{secondary_species}":
            ...
            array_X_in_Fe = df.loc[mask, f"array_X_{secondary_species}"].iloc[0]

        # fn_estimate__X_in_MgSiO3 = lambda array_X_in_Fe, KD__part: 5 / ( (KD__part/array_X_in_Fe) + 4)
        array_X_in_MgSiO3 = 5 / ( (KD__part/array_X_in_Fe) + 4)

        if phase == f"Fe_{secondary_species}":
            df.loc[mask, f"array_X_{secondary_species}"] = array_X_in_MgSiO3
            # row__array_X_in_MgSiO3 = np.nan * np.ones_like(array_X_in_Fe)  # fill with NaN -- i.e., if Fe_{secondary_species} phase, then we do not have X_in_MgSiO3
        else:
            # row__array_X_in_MgSiO3 = array_X_in_MgSiO3
            df.at[i, f"array_X_{secondary_species}"] = array_X_in_MgSiO3
    ...
    for i, row in df_superset.iterrows():
        ...
        if phase == f"Fe_{secondary_species}":
            ...
            array_X_in_Fe = df_superset.loc[mask, f"array_X_{secondary_species}"].iloc[0]

        # fn_estimate__X_in_MgSiO3 = lambda array_X_in_Fe, KD__part: 5 / ( (KD__part/array_X_in_Fe) + 4)
        array_X_in_MgSiO3 = 5 / ( (KD__part/array_X_in_Fe) + 4)

        if phase == f"Fe_{secondary_species}":
            df_superset.loc[mask, f"array_X_{secondary_species}"] = array_X_in_MgSiO3
            # row__array_X_in_MgSiO3 = np.nan * np.ones_like(array_X_in_Fe)  # fill with NaN -- i.e., if Fe_{secondary_species} phase, then we do not have X_in_MgSiO3
        else:
            # row__array_X_in_MgSiO3 = array_X_in_MgSiO3
            df_superset.at[i, f"array_X_{secondary_species}"] = array_X_in_MgSiO3
