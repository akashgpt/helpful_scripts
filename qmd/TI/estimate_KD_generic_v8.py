...
Estimate partition coefficients (K_D) and weight-based distribution coefficients
(D_wt) for a dilute secondary species (e.g. He, H) partitioning between liquid Fe
(metal) and MgSiO3 (silicate) at high P-T conditions, using data from
thermodynamic integration (TI) and Gibbs-Helmholtz (GH) free-energy analyses of
ab-initio molecular dynamics (AIMD) simulations.

Physical background
-------------------
The script implements the thermodynamic framework described in:
...
# Monte Carlo error propagation utilities (see $HELP_SCRIPTS/general/mc_error.py)
# - monte_carlo_error:              symmetric Gaussian sampling, returns (mean, std)
# - monte_carlo_error_asymmetric:   asymmetric bounds, returns (mean, std, lower, upper)
# - *_w_bounds / *_w_io_bounds:     variants that enforce input/output bounds
# - *_vectorized_outputs:           variant for functions returning multi-column arrays
from mc_error import monte_carlo_error
from mc_error import monte_carlo_error_asymmetric
from mc_error import monte_carlo_error_asymmetric_w_bounds
from mc_error import monte_carlo_error_asymmetric_w_io_bounds
from mc_error import monte_carlo_error_asymmetric_w_io_bounds_vectorized_outputs
from scipy.optimize import curve_fit

###############################################################################
# PHYSICAL CONSTANTS & ATOMIC MASSES
###############################################################################
kB = 8.617333262145e-5       # Boltzmann constant [eV/K] — used for entropy terms
mu_MgSiO3 = 100.0            # Molar mass of MgSiO3 formula unit [g/mol] (Mg:24.305 + Si:28.086 + 3*O:15.999)
mu_Fe = 55.845               # Molar mass of Fe [g/mol]
mu_H = 1.00784               # Molar mass of H [g/mol]
mu_H2 = 2 * mu_H             # Molar mass of H2 [g/mol]
mu_He = 4.002602              # Molar mass of He [g/mol]
epsilon = 1e-10               # Small floor value to prevent division-by-zero or log(0) errors
num_array_X = 100             # Number of points in the log-spaced X array (mole fraction grid)
max_array_X = 0.2             # Upper limit of X grid (~0.2 mole fraction)

###############################################################################
# SCRIPT CONTROL FLAGS
# These flags determine which parts of the script execute. Useful for
# separating the (expensive) analysis phase from the (fast) plotting phase.
###############################################################################
SCRIPT_MODE             = 1   # >0: only plot (reads CSVs); <0: only analysis; 0: both
PLOT_MODE               = 31  # -1: all plots; 0: none; N: only plot #N (see STEP 12)
TEMPERATURES_TO_REMOVE  = [10400, 7200, 5200]  # T values to exclude (likely crystalline Fe at these conditions)
FIT_MODE                = 0   # 1: overlay symbolic-regression fit on plots; 0: skip
H_STOICH_MODE           = 1   # 1: treat H as monatomic; 2: treat as H2 (affects K_D' definition)
...
# Snapshot the default matplotlib rcParams so we can reset between plot sections
INITIAL_RCP = plt.rcParams.copy()

# --- Command-line argument parsing ---
# The secondary species (e.g. He, H, C) is the dilute solute whose partitioning
# between Fe (metal) and MgSiO3 (silicate) we want to characterize.
...
# Working directory — the script expects to be run from the parent of
# Fe_{species}/ and MgSiO3_{species}/ directories.
...
# Root directories containing simulation data for each phase.
# Each contains P_T sub-folders (e.g. P50_T3500/) with configuration sub-folders.
ROOT_DIRS = [
...
###############################################################################
# ========== ANALYSIS SECTION (SCRIPT_MODE <= 0) ==========
# This block performs the full data-processing pipeline:
#   - Parse log files -> DataFrame
#   - Compute mole fractions, Gibbs energies, entropy corrections
#   - Fit excess chemical potentials
#   - Compute K_D, D_wt with Monte Carlo error propagation
#   - Incorporate Gibbs-Helmholtz isobar data
#   - Write output CSVs
###############################################################################
if SCRIPT_MODE <= 0:

    # --- Validate that the expected phase directories exist ---
    ...
    ###########################################################################
    # STEP 1: PARSE log.Ghp_analysis FILES
    ###########################################################################

    # Regex to match "Key : Value" lines in log.Ghp_analysis files.
    # These files are produced by the TI analysis pipeline and contain
    # key results like G_hp, G_hp_error, atom counts, species lists, etc.
    KV_RE = re.compile(r'^\s*([^:]+?)\s*:\s*(.+)$')

    # Helper regexes for type conversion of parsed values
    int_re   = re.compile(r'-?\d+')                      # integer tokens
    float_re = re.compile(r'[-+]?\d*\.?\d+|\d+')         # float tokens
    str_re   = re.compile(r"'([^']*)'|\"([^\"]*)\"")     # quoted string tokens

    def parse_log(path):
        """
        Read a log.Ghp_analysis file and extract all key: value pairs.

        The log file format is a series of lines like:
            G_hp : -123.456
            Atom counts : [4, 12, 4, 1]
            Target pressure : 50.0

        Returns a dict of {key: raw_string_value} for further type conversion.
        """
        out = {}
    ...
    # --- Type-conversion helpers for parsed log values ---
    # The parse_log function returns raw strings; these functions convert them
    # to Python types (int lists, float lists, string lists, scalars).
    ...
    ###########################################################################
    # STEP 1 (cont.): TRAVERSE DIRECTORY TREE AND COLLECT DATA
    # Walk Fe_{species}/ and MgSiO3_{species}/ -> P_T_folder/ -> Config/
    # and parse each log.Ghp_analysis into a record dict.
    ###########################################################################
    records = []
    for phase in ROOT_DIRS:
        for pt_dir in Path(phase).iterdir():            # e.g. Fe_He/P50_T3500/
            if not pt_dir.is_dir():
                continue
            for cfg in pt_dir.iterdir():                # e.g. Fe_He/P50_T3500/Config1_8He/
                logf = cfg / "log.Ghp_analysis"
                if not logf.is_file():
                    continue
                rec = parse_log(logf)
                # Tag each record with its provenance (phase, P-T condition, config)
                rec["Phase"] = phase
                rec["P_T_folder"] = pt_dir.name
                rec["Config_folder"] = cfg.name
                records.append(rec)

    # Assemble all records into the master DataFrame
    df = pd.DataFrame(records)

    ###########################################################################
    # STEP 2: CLEAN AND CONVERT COLUMN TYPES
    # The raw values from parse_log are all strings; convert list-like columns
    # (atom counts, masses, species) and scalar columns to proper Python types.
    ###########################################################################
    ...
    # Extract numeric P and T from the "Target pressure" / "Target temperature"
    # string columns (e.g. "50.0 GPa" -> 50.0) and create properly-named columns.
    ...
    ###########################################################################
    # STEP 3: INITIALIZE COMPOSITION-DEPENDENT ARRAY COLUMNS
    # For the "exact" K_D calculation, we evaluate K_D, D_wt, K_D', X_w, etc.
    # as a function of silicate mole fraction X on a log-spaced grid from
    # 1e-10 to max_array_X. Each DataFrame row stores a NumPy array for each
    # of these quantities, enabling composition-dependent analysis.
    ###########################################################################

    # Log-spaced mole fraction grid: X_{secondary_species} in silicate
    arr_X = np.logspace(np.log10(1e-10), np.log10(max_array_X), num=num_array_X)

    arr_inf = np.nan * np.ones_like(arr_X)  # NaN template for derived quantities
    arr_PT = np.nan * np.ones_like(arr_X)   # NaN template for P/T arrays

    # Define which columns get the X grid vs NaN initialization:
    #   - array_X_*:        seeded with the log-spaced mole fraction grid
    #   - array_Xw_*, array_KD_*, array_D_wt_*, array_KD_prime_*: NaN (computed later)
    #   - array_T_*, array_P_*: NaN (filled when computing exact K_D)
    #   - array_X_*_in_Fe*: seeded with X grid (later overwritten with Fe mole fractions)
    array_cols = {
    ...
    # Broadcast each template array to every row (object-dtype columns)
    ...
    # Sort by phase and pressure for consistent ordering in all downstream operations
    ...
    ###########################################################################
    # STEP 4: COMPUTE MOLE FRACTIONS X_{secondary_species}
    # Definition: X = n_secondary / (n_primary + n_secondary)
    #   - For Fe phase:     n_primary = number of Fe atoms
    #   - For MgSiO3 phase: n_primary = number of Mg atoms (one per formula unit)
    # This counts "species units" not total atoms, so O and Si in MgSiO3
    # are excluded from the denominator.
    ###########################################################################

    def frac_secondary_species(row):
        """Compute mole fraction X of the secondary species in the simulation cell."""
        ...
        n_sec = mapping.get(secondary_species, 0)  # number of secondary atoms

        # Select the primary species count based on the phase
        phase = row["Phase"]
        if phase == f"Fe_{secondary_species}":
            n_prim = mapping.get("Fe", 0)
        elif phase == f"MgSiO3_{secondary_species}":
            n_prim = mapping.get("Mg", 0)     # Mg count = number of MgSiO3 formula units
        else:
            return 0.0
    ...
    # Compute "Total # of species" = n_primary + n_secondary for each row.
    # This is the number of independent "species units" (Fe atoms or MgSiO3
    # formula units, plus secondary atoms), NOT the total atom count.
    # It is used to normalize G_hp to a per-species basis for the linear fit.
    ...
    # Drop any informational WARNING columns that were parsed from log files
    ...
    ###########################################################################
    # STEP 5: COMPUTE PER-SPECIES GIBBS ENERGIES WITH ENTROPY CORRECTIONS
    # G_hp from TI is the total Gibbs free energy of the cell. We normalize
    # per-atom and per-species. The TS correction accounts for the entropy
    # of mixing that is NOT captured in the TI integration (since TI gives
    # the free energy of a fixed composition, not the mixing contribution).
    #
    # For cells with X > 0 (mixed systems):
    #   G_hp_per_species_w_TS = G_hp / N_species + TS / N_species
    # For pure cells (X = 0):
    #   G_hp_per_species_w_TS = G_hp / N_species  (no TS correction)
    ###########################################################################
    ...
    # Error propagation: TS is treated as having zero uncertainty (deterministic
    # from the temperature), so G_hp_per_*_w_TS_error equals G_hp_per_*_error.
    df["G_hp_per_atom_w_TS_error"] = df["G_hp_per_atom_error"]
    df["G_hp_per_species_error"] = df["G_hp_per_atom_error"] * df["Total # of atoms"] / df["Total # of species"]
    df["G_hp_per_species_w_TS_error"] = df["G_hp_per_species_error"]

    ###########################################################################
    # STEP 6: FIT EXCESS CHEMICAL POTENTIAL mu_excess
    #
    # For each (Phase, P_T_folder) group, fit a weighted linear model:
    #     G_hp_per_species_w_TS = intercept + slope * X
    #
    # The excess chemical potential is then:
    #     mu_excess = slope + intercept
    # which is the tangent-line extrapolation to X = 1 (pure secondary species).
    #
    # We use scipy.optimize.curve_fit with absolute_sigma=True so that the
    # covariance matrix correctly reflects the input uncertainties.
    ###########################################################################
    df["intercept"] = np.nan
    df["slope"] = np.nan
    ...
    ###########################################################################
    # STEP 7: COMPUTE IDEAL-MIXING ENTROPY TERM mu_TS AND TOTAL mu
    #
    # The ideal mixing contribution to the chemical potential depends on the
    # lattice stoichiometry:
    #   Fe phase:     mu_TS = k_B * T * ln(X)
    #   MgSiO3 phase: mu_TS = k_B * T * ln(X / (5 - 4*X))
    #     The denominator (5 - 4*X) arises because MgSiO3 has 5 atoms per
    #     formula unit, and the secondary species replaces Mg sites.
    #
    # NOTE: mu_TS is computed but NOT actually used in the K_D calculation —
    #       only mu_excess (from the linear fit) enters the partition coefficient.
    #       mu and mu_TS are retained for diagnostic/comparison purposes.
    ###########################################################################
    def compute_mu_TS(T,X):
        """
        Compute the ideal-mixing entropy contribution to the chemical potential.

        For Fe phase:     mu_TS = k_B * T * ln(X)
        For MgSiO3 phase: mu_TS = k_B * T * ln(X / (5 - 4*X))
        """
    ...
    ###########################################################################
    # STEP 8: COMPUTE K_D (DILUTE-LIMIT APPROXIMATION) AND D_wt
    #
    # The partition coefficient K_D for the reaction X_silicate -> X_metal is:
    #
    #   K_D = (1/5) * exp( -(mu_excess_Fe - mu_excess_MgSiO3) / (k_B * T) )
    #
    # The factor 1/5 comes from the dilute-limit (X_sil << 1) of the exact
    # expression K_D = exp(...) / (5 - 4*X_sil). When X_sil -> 0, (5-4*X) -> 5.
    #
    # mult_factor ensures the sign convention is consistent regardless of
    # which phase's row we're evaluating: K_D is always silicate-to-metal.
    #
    # D_wt = K_D * (M_MgSiO3 / M_Fe) = K_D * (100/56) converts to weight-
    # fraction-based partition coefficient.
    ###########################################################################
    def compute_KD(row, df_evaluate=None):
        """Compute K_D (dilute-limit) for a given row by pairing with the other phase."""
        ...
        # Sign convention: mult_factor = +1 when row is Fe, -1 when row is MgSiO3,
        # so that K_D always represents the silicate -> metal direction.
        if phase == f"Fe_{secondary_species}":
            mult_factor = 1
        else:
            mult_factor = -1
    ...
    def compute_KD__exact(row, df_evaluate=None):
        """
        Compute the "exact" K_D (without the dilute-limit approximation).

        Unlike compute_KD which assumes X_sil << 1 (giving the 1/5 prefactor),
        this function retains the full (5 - 4*X_sil) denominator and also
        computes the implied X_Fe from X_MgSiO3 via the equilibrium relation.

        Returns: (KD, KD_error, KD_lower, KD_upper,
                  row__array_X_in_MgSiO3, df_evaluate)
        """
        if df_evaluate is None:
            raise ValueError("df_evaluate must be provided to compute_KD")
        phase = row["Phase"]
    ...
    # --- Apply compute_KD to every row (dilute-limit K_D) ---
    ...
    # Convert K_D (mole-fraction-based) to D_wt (weight-fraction-based)
    ...
    # Tag all rows from TI analysis (vs "GH" for Gibbs-Helmholtz isobar data)
    df["calc_type"] = "TI"
    ...
    # Normalize "Base directory" paths: replace any prefix up to and including
    # the home_dirname with the full home_dir path (handles different absolute paths)
    ...
    ###########################################################################
    # STEP 9: INCORPORATE GIBBS-HELMHOLTZ (ISOBAR) DATA
    #
    # For each (Phase, P_T_folder), look inside each Config's isobar_calc/
    # directory. If GH_analysis.csv exists, it contains Gibbs free energies
    # at additional temperatures along the same isobar (from Gibbs-Helmholtz
    # integration of enthalpy data). These extend the TI results to a wider
    # temperature range without additional TI calculations.
    #
    # For each new temperature:
    #   1. TS is linearly scaled from the TI value: TS(T_new) = TS(T_TI) * T_new/T_TI
    #   2. G_hp and G_hp_error come from GH_analysis.csv
    #   3. G_hp_per_species and G_hp_per_species_w_TS are recomputed
    #   4. A new linear fit (slope, intercept) is performed for mu_excess
    #   5. Fractional errors from the TI fit are assumed for the GH fit
    #
    # Results are accumulated in df_superset, which later merges with df.
    ###########################################################################
    ...
    ###########################################################################
    # Merge the original TI DataFrame into df_superset, then filter to only
    # configurations containing "_8H" in the name (the primary/largest cells).
    ...
    ###########################################################################
    # STEP 10: COMPUTE "EXACT" K_D ARRAYS AS A FUNCTION OF COMPOSITION
    #
    # For each MgSiO3 row in df_superset, compute K_D, K_D', D_wt, X_Fe, X_w
    # etc. across the full log-spaced X_MgSiO3 grid, with error propagation.
    #
    # The "exact" K_D differs from the dilute-limit by retaining the
    # composition-dependent denominator: K_D = exp(...) / (5 - 4*X_sil)
    #
    # This uses fn_for_all_v2 which computes all derived quantities in a
    # single vectorized function call, and
    # monte_carlo_error_asymmetric_w_io_bounds_vectorized_outputs for
    # efficient asymmetric error propagation with bounded outputs.
    ###########################################################################
    for i, row in df_superset.iterrows():
        ...
        # --- Weight-fraction conversion functions ---
        # Mole fraction X to weight fraction X_w:
        #   X_w = X / [X + (M_primary / M_secondary) * (1 - X)]
        # where M_primary is the molar mass of the primary phase formula unit
        # and M_secondary is the molar mass of the secondary species.
        # Note: in this loop, phase is always MgSiO3, so other is always Fe.
        def fn_X_to_Xw(X):
            """Convert mole fraction X to weight fraction X_w."""
            X_w = X / (X + ((mu_primary/mu_secondary)*(1-X)))
            return X_w

        def fn_Xw_to_D_wt(X_in_MgSiO3, mu_excess, other__mu_excess, T, mult_factor):
            """Compute D_wt = Xw_Fe / Xw_MgSiO3 from the equilibrium relation."""
            X_in_Fe = X_in_MgSiO3 * ( np.exp(-mult_factor * (mu_excess - other__mu_excess) / (kB * T)) ) / (5 - 4*X_in_MgSiO3)
            Xw_in_MgSiO3 = X_in_MgSiO3 / ( X_in_MgSiO3 + ((mu_MgSiO3/mu_secondary)*(1-X_in_MgSiO3)) )
            Xw_in_Fe = X_in_Fe / ( X_in_Fe + ((mu_Fe/mu_secondary)*(1-X_in_Fe)) )
            D_wt = Xw_in_Fe / Xw_in_MgSiO3
        ...
        def fn_KD_prime(X_in_MgSiO3, mu_excess, other__mu_excess, T, mult_factor):
            """
            Compute K_D' for the H2 stoichiometry reaction:
              H2 (silicate) = 2 H (metal)
            This is relevant when hydrogen is molecular in the silicate but
            monatomic in the metal. K_D' is defined via the weight-fraction
            relation appropriate for this stoichiometry.
            """
            X_in_Fe = X_in_MgSiO3 * ( np.exp(-mult_factor * (mu_excess - other__mu_excess) / (kB * T)) ) / (5 - 4*X_in_MgSiO3)
            Xw_in_MgSiO3 = X_in_MgSiO3 / ( X_in_MgSiO3 + ((mu_MgSiO3/mu_secondary)*(1-X_in_MgSiO3)) )
            Xw_in_Fe = X_in_Fe / ( X_in_Fe + ((mu_Fe/mu_secondary)*(1-X_in_Fe)) )
        ...
        def fn_for_all_v2(X_in_MgSiO3, slope_MgSiO3, intercept_MgSiO3, slope_Fe, intercept_Fe, T):
            """
            Vectorized computation of all derived thermodynamic quantities from
            the linear-fit parameters (slope + intercept = mu_excess) for both phases.

            This function is passed to the vectorized Monte Carlo error propagator,
            which samples all 6 inputs simultaneously and collects statistics on
            the 6 outputs.

            Parameters:
                X_in_MgSiO3      : mole fraction of secondary species in MgSiO3 (scalar)
                slope_MgSiO3     : linear-fit slope for MgSiO3 phase
                intercept_MgSiO3 : linear-fit intercept for MgSiO3 phase
                slope_Fe         : linear-fit slope for Fe phase
                intercept_Fe     : linear-fit intercept for Fe phase
                T                : temperature [K]

            Returns:
                (6, N_MC) array of [Xw_MgSiO3, D_wt, KD_prime, KD, X_Fe, Xw_Fe]
            """
            # Reconstruct mu_excess from the fit parameters
            mu_excess_MgSiO3 = slope_MgSiO3 + intercept_MgSiO3
            mu_excess_Fe = slope_Fe + intercept_Fe

            # Equilibrium X in Fe from the partition relation
            X_in_Fe = X_in_MgSiO3 * ( np.exp((mu_excess_MgSiO3 - mu_excess_Fe) / (kB * T)) ) / (5 - 4*X_in_MgSiO3)

            # Convert mole fractions to weight fractions for each phase
            Xw_in_MgSiO3 = X_in_MgSiO3 / ( X_in_MgSiO3 + ((mu_MgSiO3/mu_secondary)*(1-X_in_MgSiO3)) )
            Xw_in_Fe = X_in_Fe / ( X_in_Fe + ((mu_Fe/mu_secondary)*(1-X_in_Fe)) )

            # Exact K_D (composition-dependent)
            KD = ( np.exp((mu_excess_MgSiO3 - mu_excess_Fe) / (kB * T)) ) / (5 - 4*X_in_MgSiO3)

            # Weight-fraction partition coefficient
            D_wt = Xw_in_Fe / Xw_in_MgSiO3

            # K_D' for the H2-stoichiometry reaction (H2_sil = 2H_metal)
            KD_prime = (1 + ( ((1/Xw_in_MgSiO3)-1) * (mu_H2/mu_MgSiO3) ) ) / ((1 + ( ((1/Xw_in_Fe)-1) * (mu_H/mu_Fe) ) )**2)

            # Stack all 6 outputs for the vectorized MC error propagator
    ...
    ###########################################################################
    # STEP 11: CLEAN UP AND WRITE OUTPUT CSVs
    ###########################################################################

    # Remove data points at temperatures where Fe is likely crystalline (not liquid)
    ...
    # Write the combined TI + GH superset to CSV
    ...
    # Write the TI-only results to CSV
    pd.set_option("display.width", 200)
...
###############################################################################
# ========== PLOTTING SECTION (SCRIPT_MODE >= 0) ==========
# Generates publication-quality figures from the CSVs written in the analysis
# section. Each PLOT_MODE value selects a different figure type.
# PLOT_MODE < 0 generates all figures sequentially.
###############################################################################
if SCRIPT_MODE >= 0:
    ...
    ###########################################################################
    # HELPER FUNCTION: plot_studies
    # Overlays data from literature studies (computational or experimental)
    # onto the K_D and D_wt axes with temperature-dependent coloring.
    ###########################################################################
    def plot_studies(
        fn_ax_KD,
        fn_ax_D_wt,
    ...
    def _extract_col(src, col):
        """Return 1-D float array for the given column from a DataFrame/Series/dict.
        Used to uniformly access columns from heterogeneous data sources
        (DataFrames, dicts of lists, etc.) when assembling combined datasets."""
        try:
    ...
    def _concat_cols(sources, col):
        """Concatenate a column across multiple heterogeneous sources into one 1-D array.
        Useful for merging our TI data (DataFrame) with literature data (dicts)."""
        parts = []
        for s in sources:
    ...
    def parse_numpy_repr_list(s):
        """
        Parse numpy-style string representations back into float64 arrays.

        When array columns are saved to CSV via DataFrame.to_csv(), they are
        serialized as strings like "[1.0 2.5e-3  nan 4.]". This function
        parses those strings back into proper np.ndarray objects.

        Handles: space-separated or comma-separated tokens, nan, inf, scientific
        notation. Returns the original object if already an array or not a
        bracketed string.
        """
    ...
    ###########################################################################
    # LITERATURE DATA: PREVIOUS COMPUTATIONAL STUDIES
    # Each dataset is stored as a dict with keys matching DataFrame column
    # names. D_wt and K_D are converted between each other using:
    #   K_D = D_wt * (M_Fe / M_MgSiO3) = D_wt * (56/100)
    #   D_wt = K_D * (M_MgSiO3 / M_Fe) = K_D * (100/56)
    ###########################################################################
    if secondary_species == "He":
    ...
    ###########################################################################
    # LITERATURE DATA: PREVIOUS EXPERIMENTAL STUDIES
    # These are lower P-T conditions compared to the computational studies
    # above (typically < 20 GPa, < 2500 K).
    ###########################################################################
    if secondary_species == "He":
    ...
    ###########################################################################
    # LITERATURE DATA: TWO-PHASE AIMD SIMULATIONS (THIS STUDY)
    # These come from direct two-phase (metal + silicate coexistence) AIMD
    # simulations, which provide an independent check on the TI-derived K_D.
    ###########################################################################
    if secondary_species == "He":
        """ 
    ...
    ###########################################################################
    # SYMBOLIC REGRESSION FIT FUNCTIONS
    # These empirical models were found via symbolic regression (e.g. PySR)
    # to fit K_D as a function of composition (x0 = log(X)), pressure (x1 = P),
    # and temperature (x2 = T). Multiple candidate expressions are shown
    # (commented out where superseded), along with their loss values.
    #
    # best_fn:    primary fit; best_fn_v2: version used for plotting overlays.
    # The functions return exp(fn), i.e. the K_D or K_D' value.
    ###########################################################################
    def best_fn(x0, x1, x2):  # x0: log(X or X2), x1: P [GPa], x2: T [K]
    ...
    def best_fn_v2(x0, x1, x2, H_STOICH_MODE=2):  # x0: log(X or X2), x1: P [GPa], x2: T [K]
    ...
    ###########################################################################
    # STEP 12: LOAD DATA AND GENERATE PLOTS
    # Re-import libraries (safe in case analysis section was skipped via SCRIPT_MODE)
    # and load the CSV produced by the analysis phase.
    ###########################################################################
    import numpy as np
    import pandas as pd
    import matplotlib.pyplot as plt
    ...
    # Load the TI-only results CSV (produced by the analysis section or a previous run)
    ...
    if PLOT_MODE == 1:
        # ===================================================================
        # PLOT 1: G_hp_per_species_w_TS vs X_{secondary_species}
        # Shows the linear fit of Gibbs energy per species unit as a function
        # of mole fraction, with points colored by P_T_folder and sized by phase.
        # This is the fundamental diagnostic plot for verifying the linear fit
        # that yields mu_excess.
        # ===================================================================

        # Build a discrete colormap only for the P_T_folder categories actually present
    ...
    if PLOT_MODE == 2:
        # ===================================================================
        # PLOT 2: mu_{secondary_species} vs X_{secondary_species}
        # Shows the total chemical potential (excess + entropy) as a function
        # of mole fraction, colored by P_T_folder and sized by phase.
        # ===================================================================
    ...
    ###########################################################################
    # LOAD THE SUPERSET CSV (TI + GH data) FOR REMAINING PLOTS
    # Filter to _8H configurations and reconvert stringified arrays back to
    # numpy arrays using parse_numpy_repr_list.
    ###########################################################################
    df_superset = pd.read_csv("all_TI_results_superset.csv")
    df_superset = df_superset[df_superset["Config_folder"].str.contains("_8H")]

    # Reconvert array columns from string representation back to numpy arrays
    ...
    # Distinct markers for each data source to distinguish on the plot
    marker_TI = "o"             # this study: thermodynamic integration
    marker_2phase = "s"         # this study: two-phase AIMD simulations
    marker_other_comp_studies = ["^", "D", "v", "<", ">"]  # literature computational studies
    marker_other_expt_studies = ["p", "x", "h", "*", "H"]  # literature experimental studies

    # Scatter/errorbar styling: "this study" uses larger, fully opaque markers;
    # literature data uses smaller, semi-transparent markers to avoid visual clutter.
    marker_opts = dict(marker='o', linestyle='', markersize=10, alpha=1)
    marker_opts_scatter = dict(linestyle='', s=200, alpha=1)
    marker_opts_error = dict(linestyle='', markersize=10, alpha=1, capsize=3, elinewidth=1)
    marker_opts_scatter__others = dict(linestyle='', s=100, alpha=0.5)
    marker_opts_error__others = dict(linestyle='', markersize=10, capsize=3, elinewidth=1, alpha=0.5)
    ...
    if PLOT_MODE == 3 or PLOT_MODE < 0:
        # ===================================================================
        # PLOT 3: K_D and D_wt vs Pressure, colored by Temperature
        # Two-panel figure (K_D top, D_wt bottom) with log y-scale.
        # Uses pastel magma colormap for temperature encoding.
        # Overlays this study's TI data, literature computational studies,
        # and literature experimental studies.
        # Output: KD_D_wt_vs_P_T__{secondary_species}.png
        # ===================================================================
    ...
    if PLOT_MODE == 4 or PLOT_MODE < 0:
        # ===================================================================
        # PLOT 4: K_D and D_wt vs Temperature, colored by Pressure
        # Two-panel figure + a separate 4-panel figure showing K_D vs T
        # for each unique P_T_folder separately.
        # Uses pastel viridis colormap for pressure encoding.
        # Output: KD_D_wt_vs_P_T__T__{secondary_species}.png
        #         KD_sil_to_metal_vs_P_T_panels__{secondary_species}.png
        # ===================================================================
    ...
    if PLOT_MODE == 5 or PLOT_MODE < 0:
        # ===================================================================
        # PLOT 5: K_D and D_wt vs Pressure — LOW-P REGIME (< 70 GPa)
        # Same as PLOT 3 but zoomed to the low-pressure regime where
        # experimental data exists, for direct comparison.
        # Output: KD_D_wt_vs_P_T__lowPT__{secondary_species}.png
        # ===================================================================
    ...
    if PLOT_MODE == 6 or PLOT_MODE < 0:
        # ===================================================================
        # PLOT 6: K_D and D_wt vs Temperature — LOW-P REGIME
        # Same as PLOT 4 but zoomed to low-T (< 5500 K) and low-P (< 70 GPa).
        # Output: KD_D_wt_vs_P_T__T__lowPT__{secondary_species}.png
        # ===================================================================
    ...
    if PLOT_MODE == 7 or PLOT_MODE < 0:
        # ===================================================================
        # PLOT 7: P-T PHASE SPACE SCATTER, colored by K_D
        # Shows all data points in P-T space with color encoding log(K_D).
        # Both axes are log-scaled. Overlays computational and experimental
        # literature data.
        # Output: KD_sil_to_metal__PT__{secondary_species}.png
        # ===================================================================
    ...
    if PLOT_MODE == 8 or PLOT_MODE < 0:
        # ===================================================================
        # PLOT 8: COMPOSITION-DEPENDENT K_D, K_D', D_wt PANELS
        # Four multi-panel figures showing how K_D, K_D', and D_wt vary as a
        # function of silicate mole fraction X (and weight fraction Xw).
        # Each figure is a 4x5 grid: rows = pressures, cols = temperatures.
        # Horizontal lines show the dilute-limit K_D and D_wt (with errors).
        # Shaded bands show Monte Carlo error envelopes.
        # Output: 4 PNG files (KD_vs_X, KD_prime_vs_X, D_wt_vs_X, D_wt_vs_Xw)
        # ===================================================================
    ...
    if PLOT_MODE == 15 or PLOT_MODE < 0:
        # ===================================================================
        # PLOT 15: K_D and D_wt vs Pressure — LOW-P, LITERATURE DATA ONLY
        # Same as PLOT 5 but showing only computational and experimental
        # literature data (no TI data from this study).
        # Output: KD_D_wt_vs_P_T__lowPT__only_past_data__{secondary_species}.png
        # ===================================================================
    ...
    if PLOT_MODE == 16 or PLOT_MODE < 0:
        # ===================================================================
        # PLOT 16: K_D and D_wt vs Temperature — LOW-P, LITERATURE DATA ONLY
        # Same as PLOT 6 but without this study's TI data.
        # Output: KD_D_wt_vs_P_T__T__lowPT__{secondary_species}__past_data_only.png
        # ===================================================================
    ...
    if PLOT_MODE == 21 or PLOT_MODE < 0:
        # ===================================================================
        # PLOT 21: PUBLICATION FIGURE — K_D and D_wt vs Pressure
        # Four-panel layout: low-P and high-P side by side for K_D and D_wt.
        # Uses horizontal colorbar for temperature.
        # Output: paper__KD_D_wt__vs_P_T__{secondary_species}.png/svg
        # ===================================================================
    ...
    if PLOT_MODE == 22 or PLOT_MODE < 0:
        # ===================================================================
        # PLOT 22: PUBLICATION FIGURE — K_D and D_wt vs Temperature
        # Four-panel layout with pressure-colored data.
        # Output: paper__KD_D_wt__vs_P_T__T__{secondary_species}.png/svg
        # ===================================================================
    ...
    if PLOT_MODE == 31 or PLOT_MODE < 0:
        # ===================================================================
        # PLOT 31: PUBLICATION-READY K_D AND D_wt FIGURE
        # Four-panel layout using GridSpec:
        #   Top row:    K_D  vs P (low-P left, high-P right)
        #   Bottom row: D_wt vs P (low-P left, high-P right)
        # Shared y-axes within each row, vertical colorbar on the right.
        # Overlays TI data, 2-phase data, and all literature studies.
        # Includes a best-fit model overlay when FIT_MODE == 1.
        # Output: paper__KD_D_wt__vs_P_T__T__{secondary_species}__v_cbar.png/svg
        # ===================================================================
        import matplotlib.gridspec as gridspec
