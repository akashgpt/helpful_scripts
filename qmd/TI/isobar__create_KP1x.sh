...
while IFS= read -r -d '' parent; do
    ...
    for child in "$parent"/KP1*; do
        if [ -d "$child" ]; then
            ...
            # cp $KP1_dir/POTCAR           $child_abs/POTCAR
            ...
            if [[ "$CONFIG_dir_name" == *_0* && "$COMPOSITION_dir_name" == *"MgSiO3"* ]]; then
                ...
                cp $SETUP_dir/POTCAR_MgSiO3 "$child_abs/POTCAR"
            elif [[ "$CONFIG_dir_name" == *_0* && "$COMPOSITION_dir_name" == *"Fe"* ]]; then
                echo "Copying POTCAR for Fe"
                cp $SETUP_dir/POTCAR_Fe "$child_abs/POTCAR"
            else
                echo "Copying default POTCAR"
                cp $SETUP_dir/POTCAR "$child_abs/POTCAR"
            fi

            cp $SETUP_dir/KPOINTS_111         $child_abs/KPOINTS   # KPOINTS 1×1×1
            cp ${SETUP_dir}/INCAR_SCALEE $child_abs/INCAR

            # Populate INCAR placeholders
            sed -i "s/__TEMP_CHOSEN__/${TEMP_CHOSEN_ISOBAR}/" $child_abs/INCAR
