...
SCRIPT_MODE = 0 # >0, only plot; <0, only do analysis; 0: both
PLOT_MODE=8 #-1: plot all; 0: do not plot, 1: plot #1, 2: plot #2, 3: plot #3 ...
...
if SCRIPT_MODE <= 0:
    ...
    # # df -- create df["array_X_{secondary_species}"] -- log varying X from 1e-10 to 0.5
    # arr = np.logspace(np.log10(1e-10), np.log10(0.5), num=100)
    # # Method B: via pandas.Series (more explicit about dtype=object)
    # df[f"array_X_{secondary_species}"] = pd.Series(
    #     [arr for _ in range(len(df))],
    #     index=df.index,
    #     dtype="object"
    # )
    # df[f"array_X_{secondary_species}_error"] = pd.Series(
    #     [arr for _ in range(len(df))],
    #     index=df.index,
    #     dtype="object"
    # )
    # df[f"array_X_{secondary_species}_lower"] = pd.Series(
    #     [arr for _ in range(len(df))],
    #     index=df.index,
    #     dtype="object"
    # )
    # df[f"array_X_{secondary_species}_upper"] = pd.Series(
    #     [arr for _ in range(len(df))],
    #     index=df.index,
    #     dtype="object"
    # )

    # # similarly define array_Xw_{secondary_species} -- Weight fraction corresponding to array_X_{secondary_species}
    # df[f"array_Xw_{secondary_species}"] = pd.Series( 
    #     [np.nan for _ in range(len(df))],
    #     index=df.index,
    #     dtype="object"
    # )
    # df[f"array_Xw_{secondary_species}_error"] = pd.Series(
    #     [np.nan for _ in range(len(df))],
    #     index=df.index,
    #     dtype="object"
    # )
    # df[f"array_Xw_{secondary_species}_lower"] = pd.Series(
    #     [np.nan for _ in range(len(df))],
    #     index=df.index,
    #     dtype="object"
    # )
    # df[f"array_Xw_{secondary_species}_upper"] = pd.Series(
    #     [np.nan for _ in range(len(df))],
    #     index=df.index,
    #     dtype="object"
    # )
    ...
    # # similarly, array_KD, array_KD_error, array_KD_lower, array_KD_upper
    # df[f"array_KD_{secondary_species}"] = pd.Series(
    #     [np.nan for _ in range(len(df))],
    #     index=df.index,
    #     dtype="object"
    # )
    # df[f"array_KD_{secondary_species}_error"] = pd.Series(
    #     [np.nan for _ in range(len(df))],
    #     index=df.index,
    #     dtype="object"
    # )
    # df[f"array_KD_{secondary_species}_lower"] = pd.Series(
    #     [np.nan for _ in range(len(df))],
    #     index=df.index,
    #     dtype="object"
    # )
    # df[f"array_KD_{secondary_species}_upper"] = pd.Series(
    #     [np.nan for _ in range(len(df))],
    #     index=df.index,
    #     dtype="object"
    # )

    # #similarly for array_D_wt, ...
    # df[f"array_D_wt_{secondary_species}"] = pd.Series(
    #     [np.nan for _ in range(len(df))],
    #     index=df.index,
    #     dtype="object"
    # )
    # df[f"array_D_wt_{secondary_species}_error"] = pd.Series(
    #     [np.nan for _ in range(len(df))],
    #     index=df.index,
    #     dtype="object"
    # )
    # df[f"array_D_wt_{secondary_species}_lower"] = pd.Series(
    #     [np.nan for _ in range(len(df))],
    #     index=df.index,
    #     dtype="object"
    # )
    # df[f"array_D_wt_{secondary_species}_upper"] = pd.Series(
    #     [np.nan for _ in range(len(df))],
    #     index=df.index,
    #     dtype="object"
    # )

    # df[f"array_KD_prime_{secondary_species}"] = pd.Series(
    #     [np.nan for _ in range(len(df))],
    #     index=df.index,
    #     dtype="object"
    # )
    # df[f"array_KD_prime_{secondary_species}_error"] = pd.Series(
    #     [np.nan for _ in range(len(df))],
    #     index=df.index,
    #     dtype="object"
    # )
    # df[f"array_KD_prime_{secondary_species}_lower"] = pd.Series(
    #     [np.nan for _ in range(len(df))],
    #     index=df.index,
    #     dtype="object"
    # )
    # df[f"array_KD_prime_{secondary_species}_upper"] = pd.Series(
    #     [np.nan for _ in range(len(df))],
    #     index=df.index,
    #     dtype="object"
    # )
    ...
    arr_X = np.logspace(np.log10(1e-10), np.log10(100/109), num=100)
    ...
    #         array_X_in_MgSiO3 = row[f"array_X_{secondary_species}"]
    #     else:
    #         array_X_in_MgSiO3 = df.loc[mask, f"array_X_{secondary_species}"].iloc[0]
    ...
    #     # if phase == f"MgSiO3_{secondary_species}":
    #     #     df.loc[mask, f"array_X_{secondary_species}"] = array_X_in_Fe
    #     #     # row__array_X_in_MgSiO3 = np.nan * np.ones_like(array_X_in_Fe)  # fill with NaN -- i.e., if Fe_{secondary_species} phase, then we do not have X_in_MgSiO3
    #     # else:
    #     #     # row__array_X_in_MgSiO3 = array_X_in_MgSiO3
    #     #     df.at[i, f"array_X_{secondary_species}"] = array_X_in_Fe
    ...
    #             [KD__part_lower, X_in_MgSiO3 - 0.0], # X_in_MgSiO3 does not have error, so we can use 0.0!
    #             [KD__part_upper, X_in_MgSiO3 + 0.0],
    #         )
    #         array_KD[j] = KD
    ...
    #         X_in_Fe, X_in_Fe_error, X_in_Fe_lower, X_in_Fe_upper = monte_carlo_error_asymmetric(
    #             fn_estimate__X_in_Fe,
    #             [KD__part, X_in_MgSiO3],
    #             [KD__part_lower, X_in_MgSiO3 - 0.0],
    #             [KD__part_upper, X_in_MgSiO3 + 0.0],
    #         )
    ...
    #     df.at[i, f"array_KD_{secondary_species}"] = array_KD
    #     df.at[i, f"array_KD_{secondary_species}_error"] = array_KD_error
    #     df.at[i, f"array_KD_{secondary_species}_lower"] = array_KD_lower
    #     df.at[i, f"array_KD_{secondary_species}_upper"] = array_KD_upper

    #     if phase == f"MgSiO3_{secondary_species}":
    #         df.loc[mask, f"array_X_{secondary_species}"] = array_X_in_Fe
    #         df.loc[mask, f"array_X_{secondary_species}_error"] = array_X_in_Fe_error
    #         df.loc[mask, f"array_X_{secondary_species}_lower"] = array_X_in_Fe_lower
    #         df.loc[mask, f"array_X_{secondary_species}_upper"] = array_X_in_Fe_upper
    #     else:
    #         df.at[i, f"array_X_{secondary_species}"] = array_X_in_Fe
    #         df.at[i, f"array_X_{secondary_species}_error"] = array_X_in_Fe_error
    #         df.at[i, f"array_X_{secondary_species}_lower"] = array_X_in_Fe_lower
    #         df.at[i, f"array_X_{secondary_species}_upper"] = array_X_in_Fe_upper
    ...
    for col, fill in array_cols.items():
        # [fill] * len(df) creates a list of length df with the same object/value
        # object-dtype is inferred automatically when the fill is an array
        df[col] = pd.Series([fill] * len(df_superset), index=df_superset.index, dtype="object")
    ...
    for i, row in df_superset.iterrows():
        ...
        # 2) Determine the other phase
        other_phase = f"Fe_{secondary_species}"
        ...
        mu_excess__MgSiO3 = mu_excess
        mu_excess__Fe = other__mu_excess
        array_X_in_MgSiO3 = row[f"array_X_{secondary_species}"]
        ...
        # CONSTANTS:                        mu_MgSiO3, mu_Fe, kB, mu_secondary, X_in_MgSiO3, T
        # INPUTS:                           mu_excess__MgSiO3, mu_excess__Fe
        # INTERMEDIATE INPUTS (OUTPUTS):    KD__part, X_in_Fe, Xw_in_Fe
        # OUTPUTS:                          Xw_in_MgSiO3, KD, D_wt, KD_prime

        X_in_MgSiO3 = array_X_in_MgSiO3  # this is the X_{secondary_species} in MgSiO3
        KD__part = np.exp( (mu_excess__MgSiO3 - mu_excess__Fe) / (kB * T))
        KD = KD__part / (5 - 4*X_in_MgSiO3)
        ...
        # # input: mu_excess__MgSiO3, mu_excess__Fe, T
        # fn_KD__part = lambda mu_excess__MgSiO3, mu_excess__Fe, T : np.exp( (mu_excess__MgSiO3 - mu_excess__Fe) / (kB * T))
        # # output: KD__part

        # # input: input + X_in_MgSiO3
        # fn_estimate__X_in_Fe = lambda KD__part, X_in_MgSiO3: (X_in_MgSiO3/(5 - 4*X_in_MgSiO3)) * KD__part
        # # output: X_in_Fe
        ...
        # # input: input
        # fn_X_to_Xw_in_MgSiO3 = lambda X_in_MgSiO3: X_in_MgSiO3 / ( X_in_MgSiO3 + ((mu_MgSiO3/mu_secondary)*(1-X_in_MgSiO3)) )  # Xw = X / (1 + X)
        # fn_X_to_Xw_in_Fe = lambda X_in_Fe: X_in_Fe / ( X_in_Fe + ((mu_Fe/mu_secondary)*(1-X_in_Fe)) )  # Xw = X / (1 + X)
        # # output: Xw_in_MgSiO3, Xw_in_Fe

        # # input: input + Xw_in_MgSiO3, Xw_in_Fe
        # fn_Xw_to_D_wt = lambda Xw_in_MgSiO3, Xw_in_Fe: Xw_in_Fe / Xw_in_MgSiO3  # D_wt = Xw_in_Fe / Xw_in_MgSiO3
        # # output: D_wt

        # # input: input
        # fn_for_KD_prime = lambda Xw_in_MgSiO3, Xw_in_Fe: (1 + ( ((1/Xw_in_MgSiO3)-1) * (mu_H2/mu_MgSiO3) ) ) / ((1 + ( ((1/Xw_in_Fe)-1) * (mu_H/mu_Fe) ) )**2)
        # # output: KD_prime
...
if PLOT_MODE == 8 or PLOT_MODE < 0:
    ...
    marker_opts = dict(marker='o', linestyle='', markersize=10, alpha=1)#,color=base_color)
    marker_opts_scatter = dict(linestyle='', s=25, alpha=0.5)#,edgecolor='black',
    ...
    for i, row in df_superset.iterrows():
        ...
        axes_1[i_axes].errorbar(
            array_X, array_KD,
            yerr=[array_KD_lower, array_KD_upper],
            fmt='none',  # no extra marker
        ...
        # error
        axes_1[i_axes].axhline(
            y=row["KD_sil_to_metal"] - row["KD_sil_to_metal_low"],
            color='black', linestyle=':', linewidth=1,
            # label=f"K$_D$ error = {row['KD_sil_to_metal_error']:.2e}"
        )
        axes_1[i_axes].axhline(
            y=row["KD_sil_to_metal"] + row["KD_sil_to_metal_high"],
            color='black', linestyle=':', linewidth=1,
        )
        ########################
        ...
        axes_2[i_axes].errorbar(
            array_X, array_KD_prime,
            yerr=[array_KD_prime_lower, array_KD_prime_upper],
            fmt='none',  # no extra marker
            # ecolor=cmap(norm(row["Target temperature (K)"])),  # single RGBA tuple
            **marker_opts_error,
            marker=marker_TI  # use the TI marker for errorbars
        ...
        #     array_X, array_D_wt,
        #     yerr=[array_D_wt_lower, array_D_wt_upper],
        #     fmt='none',  # no extra marker
        #     # ecolor=cmap(norm(row["Target temperature (K)"])),  # single RGBA tuple
        #     **marker_opts_error,
        #     marker=marker_TI  # use the TI marker for errorbars
        ...
        # error
        axes_3[i_axes].axhline(
            y=row["D_wt"] - row["D_wt_low"],
            color='black', linestyle=':', linewidth=1,
            # label=f"D$_{{wt}}$ error = {row['D_wt_error']:.2e}"
        )
        axes_3[i_axes].axhline(
            y=row["D_wt"] + row["D_wt_high"],
            color='black', linestyle=':', linewidth=1,
        )
        ########################
    ...
    for axes in [axes_1, axes_2, axes_3, axes_4]:
    ...
    fig_2.savefig(f"array__KD_vs_X_prime.png", dpi=300)
