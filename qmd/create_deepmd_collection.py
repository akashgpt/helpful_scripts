...
def configure_logging(output_log_file, enable_debug=False):
    ...
    print(f"Log level (10:DEBUG | 20:INFO): {log_level}")
...
def search_bad_indices_for_keyword(file_path, keyword, output_file=None):
    ...
    # subtract 1 from all numbers in the second column -- same as dp_test_id_e_or/and_f/optimum_range files which are [0, N-1] for files going from [1, N]
...
def process_base_dir(base_dir, run_prefix):
    ...
    columns = ["runID_dir", "n_frames", "n_atoms", "n_Mg", "n_Si", "n_O", "n_H", "n_N"] + [keyword.strip() for keyword in keywords]
        ...
        for directory in dirs:
            ...
            # if directory == "deepmd":
                ...
                try:
                    ...
                    # Extract the number of Mg, Si, O, H, and N atoms

                    # open file type_map.raw as pandas dataframe
                    type_map_raw = pd.read_csv(os.path.join(deepmd_path, 'type_map.raw'), sep=r'\s+', header=None)
                    type_raw = pd.read_csv(os.path.join(deepmd_path, 'type.raw'), sep=r'\s+', header=None)
                    type_map_raw.columns = ['Type']
                    type_raw.columns = ['Type']
                    # read os.path.join(deepmd_path, 'type_map.raw') in np array
                    # type_map_raw = np.loadtxt(os.path.join(deepmd_path, 'type_map.raw'))
                    # print(type_map_raw)
                    # print(type_raw)
                    
                    Mg_loc = type_map_raw.index[type_map_raw['Type']=="Mg"].values
                    # print("Mg_loc: ",Mg_loc)
                    # if Mg_loc.values not empty, then count number of times Mg_loc occurrs in type_raw
                    if Mg_loc.size != 0:
                        Mg_count = (type_raw["Type"]==Mg_loc[0]).sum()
                    else:
                        Mg_count = 0
                    # print("Mg_count: ",Mg_count)

                    Si_loc = type_map_raw.index[type_map_raw['Type']=="Si"].values
                    # print("Si_loc: ",Si_loc)
                    if Si_loc.size != 0:
                        Si_count = (type_raw["Type"]==Si_loc[0]).sum()
                    else:
                        Si_count = 0
                    # print("Si_count: ",Si_count)

                    O_loc = type_map_raw.index[type_map_raw['Type']=="O"].values
                    # print("O_loc: ",O_loc)
                    if O_loc.size != 0:
                        O_count = (type_raw["Type"]==O_loc[0]).sum()
                    else:
                        O_count = 0
                    # print("O_count: ",O_count)

                    H_loc = type_map_raw.index[type_map_raw['Type']=="H"].values
                    # print("H_loc: ",H_loc)
                    if H_loc.size != 0:
                        H_count = (type_raw["Type"]==H_loc[0]).sum()
                    else:
                        H_count = 0
                    # print("H_count: ",H_count)

                    N_loc = type_map_raw.index[type_map_raw['Type']=="N"].values
                    # print("N_loc: ",N_loc)
                    if N_loc.size != 0:
                        N_count = (type_raw["Type"]==N_loc[0]).sum()
                    else:
                        N_count = 0
                    ...
                    # check if Mg in type_map.raw, and in which row number n_row (n_row being 0 for the first row)
    ...
    total_Mg = pd.to_numeric(log_dp_test_df["n_Mg"], errors="coerce").sum()
    total_Si = pd.to_numeric(log_dp_test_df["n_Si"], errors="coerce").sum()
    total_O = pd.to_numeric(log_dp_test_df["n_O"], errors="coerce").sum()
    total_H = pd.to_numeric(log_dp_test_df["n_H"], errors="coerce").sum()
    total_N = pd.to_numeric(log_dp_test_df["n_N"], errors="coerce").sum()
    ...
    total_row = {"runID_dir": "Total", "n_frames": total_frames, "n_atoms": total_atoms, "n_Mg": total_Mg, "n_Si": total_Si, "n_O": total_O, "n_H": total_H, "n_N": total_N, **averages}
    ...
    # check the size of log_deepmd_path_df and if it is empty, then skip the next step


    # replace the old_data_systems with log_dp_test_df["runID_dir"]
    print("deepmd files being used: ",log_deepmd_path_df.shape)
    # describe
    # print(log_deepmd_path_df.describe())
    # if log_deepmd_path_df["deepmd_path"] is empty, then 
    if log_deepmd_path_df.empty:
        print("Note: No valid system.")
                ...
                if os.path.exists(search_string):
                    ...
                    if not dp_test_files_exist(deepmd_path):
                        ...
                        if update_deepmd_w_OUTCAR and not "u." in deepmd_path:
                            ...
                            string_bad_indices = search_bad_indices_for_keyword(exclude_index_file_locn, deepmd_path_name, None)

                            # show error that things need to be fixed with the extrace_deepmd command as we don't know apriori what condition (or/and) was used to create set.000/1 -- now we can use chosen_DP_TEST_INPUT_FILE.txt
                            print("##########################################")
                            print("##########################################")
                            print("##########################################")
                            print("##########################################")
                            print("")
                            print("")
                            print(f"CORRECTED BUT NOT TESTED!!! ---- Error: Need to fix the extract_deepmd command as we don't know apriori what condition (or/and) was used to create set.000/1")
                            print("")
                            print("")
                            print("##########################################")
                            print("##########################################")
                            ...
                            # read the string in file chosen_DP_TEST_INPUT_FILE.txt in recal_path -- this is the condition used to create set.000/1
                            file__chosen_DP_TEST_INPUT_FILE = os.path.join(recal_path, chosen_DP_TEST_INPUT_FILE.txt)
                            with open(file__chosen_DP_TEST_INPUT_FILE, "r") as f:
                                chosen_DP_TEST_INPUT_FILE = f.read()
                                print(f"chosen_DP_TEST_INPUT_FILE: {chosen_DP_TEST_INPUT_FILE}")

                            # os.system(f"python $local_mldp/extract_deepmd.py -d deepmd -id dp_test_id_e_and_f -ex {string_bad_indices}")
                            os.system(f"python $local_mldp/extract_deepmd.py -d deepmd -id {chosen_DP_TEST_INPUT_FILE} -ex {string_bad_indices}")

                            if run_prefix=="TRAIN":
                                # remove set.001
                                os.system("rm -r set.001")
                            elif run_prefix=="TEST":
                                # remove set.000
                                os.system("rm -r set.000")
...
def analyze_base_dir(base_dir, run_prefix):
    ...
    file_dir = os.path.dirname(all_log_dp_test)
...
if mode in [8]:
    ...
    file_dir__TRAIN = os.path.join(destination_dir_TRAIN,"TRAIN") #"/scratch/gpfs/BURROWS/akashgpt/qmd_data/MgSiOHN/setup_MLMD/reference_analysis_runs/v5_i71__all/TRAIN"
    file_dir__TEST = os.path.join(destination_dir_TEST,"TEST") #"/scratch/gpfs/BURROWS/akashgpt/qmd_data/MgSiOHN/setup_MLMD/reference_analysis_runs/v5_i71__all/TEST"
    data_log__TRAIN = pd.read_csv(os.path.join(file_dir__TRAIN, 'all_log.dp_test'))
    data_log__TEST = pd.read_csv(os.path.join(file_dir__TEST, 'all_log.dp_test'))

    # delete last row
    data_log__TRAIN = data_log__TRAIN[:-1]
    data_log__TEST = data_log__TEST[:-1]

    # print first few lines
    # print(data_log__TRAIN.head())
    # print(data_log__TEST.head())

    # print column headers
    print(data_log__TRAIN.columns)

    ####################################################################################################################
    X_NH3__proxy = data_log__TRAIN["n_N"]/(data_log__TRAIN["n_Mg"] + data_log__TRAIN["n_N"])
    n_frames__X__n_atoms = data_log__TRAIN["n_frames"]*data_log__TRAIN["n_atoms"]

    # when X_NH3__proxy=NaN, set it to -0.1
    X_NH3__proxy = X_NH3__proxy.fillna(-0.1) 

    # add to data_log__TRAIN
    data_log__TRAIN["X_NH3__proxy"] = X_NH3__proxy
    data_log__TRAIN["n_frames * n_atoms"] = n_frames__X__n_atoms
    ####################################################################################################################

    ####################################################################################################################
    X_NH3__proxy = data_log__TEST["n_N"]/(data_log__TEST["n_Mg"] + data_log__TEST["n_N"])
    n_frames__X__n_atoms = data_log__TEST["n_frames"]*data_log__TEST["n_atoms"]

    # when X_NH3__proxy=NaN, set it to -0.1
    X_NH3__proxy = X_NH3__proxy.fillna(-0.1)

    # add to data_log__TEST
    data_log__TEST["X_NH3__proxy"] = X_NH3__proxy
    data_log__TEST["n_frames * n_atoms"] = n_frames__X__n_atoms
    ####################################################################################################################

    # bins = 50#np.array([0, 0.25, 0.5, 0.75, 1.0])  # Example edges

    # ####################################################################################################################
    # ####################################################################################################################
    # # Let's say each row has a weight: the number of frames
    # weights__TRAIN = data_log__TRAIN["n_frames"]*data_log__TRAIN["n_atoms"]
    # # Create a weighted histogram
    # plt.figure(figsize=(8, 6))
    # sns.histplot(
    #     x=data_log__TRAIN["X_NH3__proxy"],
    #     weights=weights__TRAIN,
    #     # bins=50,  # choose the number of bins you want
    #     bins=bins,  # choose the number of bins you want
    #     kde=True,  # set to True if you want a kernel density curve
    #     kde_kws={'bw_adjust': 0.75}  # Adjust this to control smoothness
    # )
    # plt.xlabel("X_NH3 ~ n_N/(n_Mg+n_N)")
    # plt.ylabel("Weighted Frequency")
    # plt.title("Histogram of X_NH3 weighted by n_frames*n_atoms")

    # # overplot for the TEST data, with opacity
    # weights__TEST = data_log__TEST["n_frames"]*data_log__TEST["n_atoms"]
    # sns.histplot(
    #     x=data_log__TEST["X_NH3__proxy"],
    #     weights=weights__TEST,
    #     # bins=50,  # choose the number of bins you want
    #     bins=bins,  # choose the number of bins you want
    #     kde=True,  # set to True if you want a kernel density curve
    #     kde_kws={'bw_adjust': 0.75},  # Adjust this to control smoothness
    ...
    # plt.tight_layout()
    # plt.show()
    # plt.savefig(os.path.join(file_dir__TRAIN, 'hist_n_N_to_n_Mg__wt_frames_atoms.png'))
    # ####################################################################################################################
    # ####################################################################################################################
    ...
    # )
    # # overplot for the TEST data, with opacity
    # weights__TEST = data_log__TEST["n_frames"]
    # sns.histplot(
    #     x=data_log__TEST["X_NH3__proxy"],
    #     weights=weights__TEST,
    #     bins=bins,  # choose the number of bins you want
    ...
    # plt.xlabel("X_NH3 ~ n_N/(n_Mg+n_N)")
    # plt.ylabel("Weighted Frequency")
    # plt.title("Histogram of X_NH3 weighted by n_frames")
    # plt.tight_layout()
    # plt.show()
    # plt.savefig(os.path.join(file_dir__TRAIN, 'hist_n_N_to_n_Mg__w_frames.png'))
    ####################################################################################################################
    ####################################################################################################################

    n_bins = 50
    X_NH3__bin_edges = np.linspace(0, 1, n_bins+1)
    X_NH3__bin_centers = 0.5*(X_NH3__bin_edges[1:] + X_NH3__bin_edges[:-1])
    freq_X_NH3_wt_frames__TRAIN, _ = np.histogram(data_log__TRAIN["X_NH3__proxy"], bins=X_NH3__bin_edges, weights=data_log__TRAIN["n_frames"])
    freq_X_NH3_wt_frames__TEST, _ = np.histogram(data_log__TEST["X_NH3__proxy"], bins=X_NH3__bin_edges, weights=data_log__TEST["n_frames"])
    freq_X_NH3_wt_frames_atoms__TRAIN, _ = np.histogram(data_log__TRAIN["X_NH3__proxy"], bins=X_NH3__bin_edges, weights=data_log__TRAIN["n_frames * n_atoms"])
    freq_X_NH3_wt_frames_atoms__TEST, _ = np.histogram(data_log__TEST["X_NH3__proxy"], bins=X_NH3__bin_edges, weights=data_log__TEST["n_frames * n_atoms"])

    # plot histogram: freq_X_NH3_wt_frames__TRAIN and freq_X_NH3_wt_frames__TEST
    plt.figure(figsize=(8, 6))
    plt.bar(X_NH3__bin_centers, freq_X_NH3_wt_frames__TRAIN, width=0.02, color='blue', alpha=0.25, label='TRAIN')
    plt.bar(X_NH3__bin_centers, freq_X_NH3_wt_frames__TEST, width=0.03, color='red', alpha=0.25, label='TEST')
    plt.xlabel("X_NH3 ~ n_N/(n_Mg+n_N)")
    plt.ylabel("Weighted Frequency")
    plt.title("Histogram of X_NH3 weighted by n_frames")
    plt.legend()
    plt.tight_layout()
    plt.show()
    plt.savefig(os.path.join(file_dir__TRAIN, 'hist_X_NH3__wt_frames__TRAIN_TEST.png'))

    # plot histogram: freq_X_NH3_wt_frames_atoms__TRAIN and freq_X_NH3_wt_frames_atoms__TEST
    plt.figure(figsize=(8, 6))
    plt.bar(X_NH3__bin_centers, freq_X_NH3_wt_frames_atoms__TRAIN, width=0.02, color='blue', alpha=0.25, label='TRAIN')
    plt.bar(X_NH3__bin_centers, freq_X_NH3_wt_frames_atoms__TEST, width=0.03, color='red', alpha=0.25, label='TEST')
    plt.xlabel("X_NH3 ~ n_N/(n_Mg+n_N)")
    plt.ylabel("Weighted Frequency")
    plt.title("Histogram of X_NH3 weighted by n_frames*n_atoms")
    plt.legend()
    plt.tight_layout()
    plt.show()
    plt.savefig(os.path.join(file_dir__TRAIN, 'hist_X_NH3__wt_frames_atoms__TRAIN_TEST.png'))

    # SUM ( <E> * n_frames ) / SUM ( n_frames ) for all X_NH3_bins
    data_log__TRAIN["Energy_RMSE_per_atom * n_frames"] = data_log__TRAIN["Energy RMSE/Natoms"]*data_log__TRAIN["n_frames"]
    data_log__TEST["Energy_RMSE_per_atom * n_frames"] = data_log__TEST["Energy RMSE/Natoms"]*data_log__TEST["n_frames"]
    data_log__TRAIN["Energy_RMSE_per_atom * n_frames * n_atoms"] = data_log__TRAIN["Energy RMSE/Natoms"]*data_log__TRAIN["n_frames * n_atoms"]
    data_log__TEST["Energy_RMSE_per_atom * n_frames * n_atoms"] = data_log__TEST["Energy RMSE/Natoms"]*data_log__TEST["n_frames * n_atoms"]

    # iterate through all bins of X_NH3 and sum the corresponding data_log__TRAIN["Energy_RMSE_per_atom * n_frames"]
    # and data_log__TRAIN["n_frames"] to compute the weighted average
    weighted_avg_wt_frames__TRAIN = []
    weighted_avg_wt_frames__TEST = []
    weighted_avg_wt_frames_atoms__TRAIN = []
    weighted_avg_wt_frames_atoms__TEST = []
    for i in range(len(X_NH3__bin_centers)):
        mask__TRAIN = (data_log__TRAIN["X_NH3__proxy"] >= X_NH3__bin_edges[i]) & (data_log__TRAIN["X_NH3__proxy"] < X_NH3__bin_edges[i+1])
        mask__TEST = (data_log__TEST["X_NH3__proxy"] >= X_NH3__bin_edges[i]) & (data_log__TEST["X_NH3__proxy"] < X_NH3__bin_edges[i+1])
        weighted_avg_wt_frames__TRAIN.append(data_log__TRAIN[mask__TRAIN]["Energy_RMSE_per_atom * n_frames"].sum()/data_log__TRAIN[mask__TRAIN]["n_frames"].sum())
        weighted_avg_wt_frames__TEST.append(data_log__TEST[mask__TEST]["Energy_RMSE_per_atom * n_frames"].sum()/data_log__TEST[mask__TEST]["n_frames"].sum())
        weighted_avg_wt_frames_atoms__TRAIN.append(data_log__TRAIN[mask__TRAIN]["Energy_RMSE_per_atom * n_frames * n_atoms"].sum()/data_log__TRAIN[mask__TRAIN]["n_frames * n_atoms"].sum())
        weighted_avg_wt_frames_atoms__TEST.append(data_log__TEST[mask__TEST]["Energy_RMSE_per_atom * n_frames * n_atoms"].sum()/data_log__TEST[mask__TEST]["n_frames * n_atoms"].sum())

    # plot weighted average of Energy_RMSE_per_atom * n_frames for all X_NH3_bins as histograms
    plt.figure(figsize=(8, 6))
    plt.bar(X_NH3__bin_centers, weighted_avg_wt_frames__TRAIN, width=0.015, color='blue', alpha=0.25, label='TRAIN')
    plt.bar(X_NH3__bin_centers, weighted_avg_wt_frames__TEST, width=0.025, color='red', alpha=0.25, label='TEST')
    plt.xlabel("X_NH3 ~ n_N/(n_Mg+n_N)")
    plt.ylabel("<Energy RMSE/Natoms * n_frames>")
    plt.title("<Energy RMSE/Natoms * n_frames> vs X_NH3")
    plt.legend()
    plt.tight_layout()
    plt.show()
    plt.savefig(os.path.join(file_dir__TRAIN, 'hist_X_NH3_Energy_RMSE_per_atom__wt_frames__TRAIN_TEST.png'))

    # the stuff below is not needed -- non sensical as Energy is already per atom!!
    # # plot weighted average of Energy_RMSE_per_atom * n_frames * n_atoms for all X_NH3_bins as histograms
    # plt.figure(figsize=(8, 6))
    # plt.bar(X_NH3__bin_centers, weighted_avg_wt_frames_atoms__TRAIN, width=0.015, color='blue', alpha=0.25, label='TRAIN')
    # plt.bar(X_NH3__bin_centers, weighted_avg_wt_frames_atoms__TEST, width=0.025, color='red', alpha=0.25, label='TEST')
    # plt.xlabel("X_NH3 ~ n_N/(n_Mg+n_N)")
    # plt.ylabel("Weighted Average of Energy/Natoms * n_frames * n_atoms")
    # plt.title("Weighted Average of Energy/Natoms * n_frames * n_atoms")
    # plt.legend()
    # plt.tight_layout()
    # plt.show()
    # plt.savefig(os.path.join(file_dir__TRAIN, 'hist_X_NH3_Energy_RMSE_per_atom__wt_frames_atoms__TRAIN_TEST.png'))


    # Unique values of data_log__TRAIN['X_NH3__proxy']
    # data_log__TRAIN['RMSE Energy/Natoms * n_frames']
    # unique_X_NH3 = data_log__TRAIN['X_NH3__proxy'].unique()

    # save the data_log__TRAIN to a csv file
    # data_log__TRAIN.to_csv(os.path.join(file_dir__TRAIN, 'all_log.dp_test__w_ratios.csv'), index=False)
