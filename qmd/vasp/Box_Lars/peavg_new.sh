...
awk '{print} /Ionic Valenz/ {exit}' $1 | grep -a -A1 "Mass of Ions in am" | grep -a -v Mass | fmt -1 | tail +3 > masses
awk '{print} /ions per type/ {exit}' $1 | grep -a "ions per type" | fmt -1 | tail +5 > numbers
...
# xlist1 contains 13 columns: E, E^2, P, P^2, E*P, F, F^2, E-F, (E-F)^2, T, T^2, E_conserved, E_conserved^2
paste xelist1 xplist1 xflist1 xtlist1 xqlist1 | awk '{print $1,$2,$3,$4,$1*$3,$5,$6,($1-$5),($1-$5)*($1-$5),$7,$8,$9,$10}' > xlist1
...
if [ $nplines -ne $nflines ]; then
    echo "WARNING: number of lines in pressure and free energy are not equal" $nelines $nplines
fi
...
read -r E  sE < <( awk '{print $1}' xlist2 | flyv_new | awk '{print $1,$2}')					# Internal Energy
read -r E2 sE2 < <(awk '{print $2}' xlist2 | flyv_new | awk '{print $1,$2}')					# Internal Energy squared
read -r P  sP < <( awk '{print $3}' xlist2 | flyv_new | awk '{print $1/'$bP',$2/'$bP'}')				# Pressure
read -r P2 sP2 < <(awk '{print $4}' xlist2 | flyv_new | awk '{print $1/'$bP'/'$bP',$2/'$bP'/'$bP'}')		# Pressure squared
read -r EP sEP < <(awk '{print $5}' xlist2 | flyv_new | awk '{print $1/'$bP',$2/'$bP'}')				# Internal Energy * Pressure
read -r F  sF < <( awk '{print $6}' xlist2 | flyv_new | awk '{print $1,$2}')					# Free Energy = Internal Energy - Temperature * Electronic Entropy
read -r S  sS < <( awk '{print $8}' xlist2 | flyv_new | awk '{print $1/'$T'*'$eVK'/'$N',$2/'$T'*'$eVK'/'$N'}')	# Electronic Entropy
read -r Tk sTk < <(awk '{print $10}' xlist2 | flyv_new | awk '{print $1,$2}')					# Temperature computed from the instantaneous kinetic energy
read -r Q  sQ < <( awk '{print $12}' xlist2 | flyv_new | awk '{print $1,$2}')					# Conserved quantity
...
echo "Internal Energy+kinetic = " $Epk "+-" $sE "eV" >> analysis/peavg.out
echo "Enthalpy+kinetic = " $H "+-" $sH "eV" >> analysis/peavg.out
echo "E-TS_el = " $F "+-" $sF "eV" >> analysis/peavg.out
echo "Electronic entropy = " $S "+-" $sS "Nk_B" >> analysis/peavg.out
echo "Energy+kinetic =" $EJ "+-" $sEJ "kJ/mol-atom" >> analysis/peavg.out
echo "Enthalpy+kinetic =" $HJ "+-" $sHJ "kJ/mol-atom" >> analysis/peavg.out
