...
T=$(grep -m 1 "TEBEG  =" $1 | cut -c 1-18 | awk '{print $3}')
...
N=$(grep -m 1 "NIONS =" $1 | awk '{print $12}')
V=$(grep -m 1 "volume of cell" $1 | awk '{print $5}')

# Read atomic masses from OUTCAR and calculate mass density
awk '{print} /Ionic Valenz/ {exit}' $1 | grep -A1 "Mass of Ions in am" | grep -v Mass | fmt -1 | tail +3 > masses
awk '{print} /ions per type/ {exit}' $1 | grep "ions per type" | fmt -1 | tail +5 > numbers
...
grep "energy  without entropy=" OUTCAR_part | awk '{print $4,$4*$4}' > xelist1

# Read temperature from OUTCAR: format requires that this must be less than 100,000 K
grep "(temperature" OUTCAR_part | cut -c 57-64 | awk '{print $1,$1*$1}' > xtlist1

# Read pressure from OUTCAR
grep "Total+kin" OUTCAR_part | awk '{print ($2+$3+$4)/3.,($2+$3+$4)*($2+$3+$4)/9.}' > xplist1

# Read free energy from OUTCAR
grep "free  energy" OUTCAR_part | awk '{print $5,$5*$5}' > xflist1

# Read ETOTAL (conserved quantity) from OUTCAR
grep "total energy   ETOTAL" OUTCAR_part | awk '{print $5,$5*$5}' > xqlist1
