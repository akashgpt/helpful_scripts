...
#SBATCH --gpus-per-node=2           # GPUs per node (DELTA: up to 4; STELLAR: up to 2)
#SBATCH --ntasks-per-node=2         # 1 MPI task per GPU
#SBATCH --cpus-per-task=1            # cpu-cores per GPU (feeds OMP threads)
#SBATCH --mem=80G                  # total memory per node (~40G per GPU)
...
run_one_vasp_gpu() {
	(
		...
		# Reconstruct SLOTS array from exported string
		local -a slots_arr=($SLOTS_STR)
		local slot_idx=$(( (slot_number - 1) % ${#slots_arr[@]} ))
		IFS=':' read -r target_node gpu_id <<< "${slots_arr[$slot_idx]}"
...

		{
			...
			echo "Node: $target_node  GPU: $gpu_id  Slot: $slot_number"
			echo "# ========================================="
			echo
		} > log.run_sim
...
parallel -j "$TOTAL_GPUS" --timeout "$INDIVIDUAL_JOB_TIMEOUT" --lb --tag \
