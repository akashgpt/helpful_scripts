...
for dir in */; do
    ...
    if [ -d "$RUN_DIR_i" ] && [ -f "$RUN_DIR_i/INCAR" ]; then
        ...
        if [ "$total_keyword_files" -lt 4 ]; then
            echo "Found $to_RUN_keyword in $RUN_DIR_i"

            # if $dir contains the keyword "done_SCALEE_0" in any file's name, skip this directory
            if [ -f "$dir/done_SCALEE_0" ]; then
                echo "Skipping $dir as it contains done_SCALEE_0"
                continue
            fi
...
#N_processes=#RUN_DIRS  # number of processes to run in parallel
