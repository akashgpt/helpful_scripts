...
# ================================================================= #
# Script to plot COLVAR file generated by PLUMED
# ================================================================= #
# Usage:
...
parser.add_argument("--num_variables_2_print", "-n", type=int, default=3,
                    help="Number of variables (columns after 'time') from COLVAR to plot. 0 means all variables. Default is 3.")
...
# Copy original file to a new file, stripping "#! FIELDS " from the first line
with open(filename_OG, 'r') as f_in, open(filename_plot, 'w') as f_out:
    first_line = f_in.readline()
    f_out.write(first_line.replace('#! FIELDS ', '', 1))
    f_out.write(f_in.read())

# Read the processed file into a pandas DataFrame
try:
    df = pd.read_csv(filename_plot, sep=r'\s+', engine='python')
except Exception as e:
    print(f"Error: could not read file COLVAR: {e}")
    sys.exit(1)
...
for ax, col in zip(axes, var_columns):
    data = np.array(pd.to_numeric(df[col], errors="coerce"))
