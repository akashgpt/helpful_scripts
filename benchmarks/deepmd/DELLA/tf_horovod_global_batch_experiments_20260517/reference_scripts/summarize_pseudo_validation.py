...
def bootstrap_weighted_mean_errors(
	values: list[tuple[float, int]],
	seed_parts: tuple[str, ...],
	samples: int = BOOTSTRAP_SAMPLES,
) -> dict[str, float]:
	"""Estimate asymmetric 1- and 2-sigma errors for a weighted mean.
	...
	Returns:
		Dictionary with point, lower/upper quantiles, and asymmetric errors.
	"""
	...
	if not clean_values or not math.isfinite(point):
		return {
			"point": math.nan,
			"minus_1sigma": math.nan,
			"plus_1sigma": math.nan,
			"minus_2sigma": math.nan,
			"plus_2sigma": math.nan,
		}
	...
	q_1sigma_low = quantile(bootstrap_values, 0.1586552539)
	q_1sigma_high = quantile(bootstrap_values, 0.8413447461)
	q_2sigma_low = quantile(bootstrap_values, 0.0227501319)
	q_2sigma_high = quantile(bootstrap_values, 0.9772498681)
	return {
		"point": point,
		"minus_1sigma": max(0.0, point - q_1sigma_low),
		"plus_1sigma": max(0.0, q_1sigma_high - point),
		"minus_2sigma": max(0.0, point - q_2sigma_low),
		"plus_2sigma": max(0.0, q_2sigma_high - point),
	}
...
def empty_error_columns() -> dict[str, str]:
	"""Return empty uncertainty columns for non-aggregate rows.

	Returns:
		Mapping with all uncertainty columns set to ``NA``.
	"""
	columns: dict[str, str] = {}
	for metric_name in ["energy_rmse_per_atom", "force_rmse", "virial_rmse_per_atom"]:
		for suffix in ["minus_1sigma", "plus_1sigma", "minus_2sigma", "plus_2sigma"]:
			columns[f"{metric_name}_{suffix}"] = "NA"
	return columns


def format_error_columns(metric_name: str, errors: dict[str, float]) -> dict[str, str]:
	"""Format uncertainty columns for one metric.

	Args:
		metric_name: Metric column prefix.
		errors: Bootstrap error dictionary.

	Returns:
		Formatted TSV columns.
	"""
	return {
		f"{metric_name}_minus_1sigma": format_float(errors["minus_1sigma"]),
		f"{metric_name}_plus_1sigma": format_float(errors["plus_1sigma"]),
		f"{metric_name}_minus_2sigma": format_float(errors["minus_2sigma"]),
		f"{metric_name}_plus_2sigma": format_float(errors["plus_2sigma"]),
	}
		...
		rows.append(
			{
				...
				"energy_rmse_per_atom": format_float(energy_errors["point"]),
				"force_rmse": format_float(force_errors["point"]),
				"virial_rmse_per_atom": format_float(virial_errors["point"]),
				**format_error_columns("energy_rmse_per_atom", energy_errors),
				**format_error_columns("force_rmse", force_errors),
				**format_error_columns("virial_rmse_per_atom", virial_errors),
			}
		)
	fieldnames = [
		"row_type",
		...
		"energy_rmse_per_atom_minus_1sigma",
		"energy_rmse_per_atom_plus_1sigma",
		"energy_rmse_per_atom_minus_2sigma",
		"energy_rmse_per_atom_plus_2sigma",
		"force_rmse_minus_1sigma",
		"force_rmse_plus_1sigma",
		"force_rmse_minus_2sigma",
		"force_rmse_plus_2sigma",
		"virial_rmse_per_atom_minus_1sigma",
		"virial_rmse_per_atom_plus_1sigma",
		"virial_rmse_per_atom_minus_2sigma",
		"virial_rmse_per_atom_plus_2sigma",
	]
