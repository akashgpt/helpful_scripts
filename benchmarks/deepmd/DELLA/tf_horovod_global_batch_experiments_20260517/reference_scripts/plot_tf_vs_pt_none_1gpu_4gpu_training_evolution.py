#!/usr/bin/env python3
"""Plot none-scaling 1GPU/4GPU TF-vs-PT training-loss evolution."""
...
TF_PLOT_ROOT = SCRATCH_ROOT / "training_loss_plots_10x_20260521" / "TF"

OUTPUT_PNG = REFERENCE_ROOT / "TF_VS_PT_NONE_1GPU_4GPU_TRAINING_EVOLUTION_ROLLING_MEDIAN_MEAN_20260523.png"
OUTPUT_TSV = REFERENCE_ROOT / "TF_VS_PT_NONE_1GPU_4GPU_TRAINING_EVOLUTION_SUMMARY_20260523.tsv"
OUTPUT_MD = REFERENCE_ROOT / "TF_VS_PT_NONE_1GPU_4GPU_TRAINING_EVOLUTION_ANALYSIS_20260523.md"

RUNS: Tuple[Tuple[str, Path, str, str, str], ...] = (
	("TF", "1gpu", SCRATCH_ROOT / "runs/long_steps_10x/reuse_1gpu_10k_10x", "1GPU baseline; linear=none for one worker", "tab:blue"),
	("TF", "4gpu", SCRATCH_ROOT / "runs/none_100k/4gpu_100k_none", "none, 100k requested", "tab:green"),
...
def run_label(framework: str, gpu_label: str) -> str:
	...
	Returns:
		Legend label.
	"""
	return f"{framework} {gpu_label} none" if gpu_label == "4gpu" or framework == "PT" else f"{framework} {gpu_label} baseline"
...
def plot_curves(curves: Dict[str, Tuple[NDArray[np.float64], str]]) -> None:
	...
	fig.suptitle("TF vs PT none-scaling training evolution: 1GPU and 4GPU", fontsize=14)
	fig.tight_layout()
...
def write_summary(rows: List[Dict[str, str]]) -> None:
	...
	text = f"""# TF vs PT None-Scaling 1GPU/4GPU Training Evolution
...
The PT 1GPU/4GPU curves and TF 4GPU curve are fixed-100k `none` runs. For TF 1GPU, worker-scaling choices are equivalent because the worker-count factor is 1, so the available `reuse_1gpu_10k_10x` curve is used as the 1GPU baseline. This is the explicit `none`-schedule comparison for 1GPU/4GPU framework behavior.
...
- TF 4GPU none and PT 4GPU none are the cleanest practical comparison here; both are
...
- For 1GPU, read TF as the shared one-worker baseline: `linear` and `none` worker scaling are equivalent there.
"""
	OUTPUT_MD.write_text(text, encoding="utf-8")


def copy_to_scratch_plot_folder() -> None:
	"""Copy aggregate outputs into the scratch TF-vs-PT and TF plot folders."""
	for plot_root in (PLOT_ROOT, TF_PLOT_ROOT):
