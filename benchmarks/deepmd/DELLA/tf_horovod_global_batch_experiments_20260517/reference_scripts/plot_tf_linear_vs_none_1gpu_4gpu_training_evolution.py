#!/usr/bin/env python3
"""Plot TF 1/4GPU linear-vs-none training-loss evolution."""
...
OUTPUT_PNG = REFERENCE_ROOT / "TF_LINEAR_VS_NONE_1GPU_4GPU_TRAINING_EVOLUTION_ROLLING_MEDIAN_MEAN_20260523.png"
OUTPUT_TSV = REFERENCE_ROOT / "TF_LINEAR_VS_NONE_1GPU_4GPU_TRAINING_EVOLUTION_SUMMARY_20260523.tsv"
OUTPUT_MD = REFERENCE_ROOT / "TF_LINEAR_VS_NONE_1GPU_4GPU_TRAINING_EVOLUTION_ANALYSIS_20260523.md"

RUNS: Tuple[Tuple[str, Path, str, str], ...] = (
	("1gpu baseline", SCRATCH_ROOT / "runs/long_steps_10x/reuse_1gpu_10k_10x", "1GPU baseline; linear=none for one worker", "tab:blue"),
	("4gpu linear", SCRATCH_ROOT / "runs/long_steps_10x/reuse_4gpu_10k_10x", "linear, 100k requested", "tab:cyan"),
...
def rolling_stat(values: NDArray[np.float64], window: int, use_median: bool) -> NDArray[np.float64]:
	"""Compute a centered rolling mean or median.
...
def summarize_run(name: str, note: str, lcurve: NDArray[np.float64]) -> Dict[str, str]:
	"""Summarize one TF training curve.
...
def plot_curves(curves: Dict[str, Tuple[NDArray[np.float64], str]]) -> None:
	...
	Args:
		curves: Mapping from run label to lcurve data and color.
	"""
	...
	fig.suptitle("TF 1/4GPU training evolution: linear vs none", fontsize=14)
	fig.tight_layout()
...
def write_summary(rows: List[Dict[str, str]]) -> None:
	...
	text = f"""# TF Linear vs None 1/4GPU Training Evolution
...
- The `4gpu none` curve is the cleaner 4GPU TF curve used in the TF-vs-PT
  comparison.
- Use this plot to understand the 4GPU schedule difference relative to the shared 1GPU baseline.
"""
	OUTPUT_MD.write_text(text, encoding="utf-8")


def copy_to_scratch_plot_folder() -> None:
	"""Copy generated reference files into the scratch TF plot folder."""
...
def main() -> None:
	"""Create TF linear-vs-none 1/4GPU training evolution plot."""
