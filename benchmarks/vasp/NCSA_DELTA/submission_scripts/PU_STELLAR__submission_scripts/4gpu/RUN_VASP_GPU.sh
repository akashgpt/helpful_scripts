#!/bin/bash
## VASP 6.6.0 GPU -- PU Stellar -- 4 GPUs (KPAR=4)
## Status: WORKING (elapsed ~397 s on benchmark system, ~3.8x speedup)
## Notes:  Requires KPAR=4 in INCAR. Stellar has 2 GPUs/node, so needs 2 nodes.
